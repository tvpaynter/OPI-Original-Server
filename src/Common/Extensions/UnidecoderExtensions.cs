...
namespace StatementIQ.Extensions
{
    ...
    /// </summary>
    public static partial class UnidecoderExtensions
    {
        /// <summary>
        ...
        public static string Unidecode(this string input, int? tempStringBuilderCapacity = null)
        {
            ...
            if (input.All(x => x < 0x80)) return input;
            ...
            foreach (var c in input)
                ...
                else
                {
                    var high = c >> 8;
                    var low = c & 0xff;
                    if (Characters.TryGetValue(high, out var transliterations)) sb.Append(transliterations[low]);
                }
        ...
        public static string Unidecode(this char c)
        {
            ...
            else
            {
                var high = c >> 8;
                var low = c & 0xff;
                result = Characters.TryGetValue(high, out var transliterations) ? transliterations[low] : string.Empty;
            }
